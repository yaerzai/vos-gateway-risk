...
@Slf4j
@Component
public class UpdateAsrResultQueue extends BatchDBRunner<CallRiskProcessDataBO> {
    @Resource
    private CallRiskProcessDataService callRiskProcessDataService;
    @Autowired
    private DataService dataService;
    @Resource
    private BatchDBRunner updateAsrResultQueue;
    ...
    protected int getThreadNum() {
        return dataService.getSysParamInt(SysParamEnum.ASR_BATCH_UPDATE_DB_THREAD_NUM);
    }*/

    @Override
    protected String getRedisQueueName() {
        return "ASRUpdateAsrResultQueue";
    }
    ...
    public void batchDB(List<CallRiskProcessDataBO> list) {
        ...
        map.keySet().forEach(tableTime -> {
            ...
            int n = callRiskProcessDataService.batchUpdateCallRiskProcess(tableTime, update);
            if (n != update.size()) {
                log.info("[Insert Submit] DB异常,更新条数不等于队列数 更新:{}, 应该更新:{}", n, update.size());
                throw new BusinessException("[update Submit] DB异常,更新条数不等于队列数");
            }
        });
    }
